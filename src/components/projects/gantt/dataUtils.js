...
export function transformScheduleToGanttData(scheduleItems) {
  ...
  scheduleItems.forEach((task, index) => {
    ...
    data.push({
      ...
      progress: 0,
      open: true,
      assignees: cleanResourceValue(task.percentage || task.resource),
      resource: cleanResourceValue(task.resource),
      percentage: cleanResourceValue(task.percentage),
    ...
    subTasks.forEach((sub, subIndex) => {
      ...
      data.push({
        ...
        progress: 0,
        parent: ganttId,
        assignees: cleanResourceValue(sub.percentage || sub.resource),
        resource: cleanResourceValue(sub.resource),
        percentage: cleanResourceValue(sub.percentage),
